  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-5">
        ...
        <Link href="/scan/batch" className="btn btn-primary text-xs"><Plus size={14} /> New batch</Link>
      </div>
          ...
          <tbody>
            {(sweeps ?? []).map(s => (
              <tr key={s.id} className="border-b border-[var(--border-soft)] last:border-0 hover:bg-[var(--bg-3)]">
                <td className="px-4 py-3"><Link href={`/sweeps/${s.id}`} className="font-medium hover:underline">{s.name ?? "Sweep"}</Link></td>
            ...
            {!sweeps?.length && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-[var(--text-muted)]">No sweeps yet. <Link href="/scan/batch" className="hover:underline" style={{ color: "var(--accent)" }}>Run your first batch →</Link></td></tr>
            )}
          </tbody>
        </table>
      </div>
